...
def problem(input_file, part2=False):
    ...
    if not part2:
        ...
        target = 1000000000
        while i < target:
            if board in board_to_index.keys():
                cycles_after = board_to_index[board]
                cycle_length = i-cycles_after
                print(f"cycle found! Starts at {cycles_after} with a length of {cycle_length}")
                remaining = target - i
                board = index_to_board[cycles_after + remaining % cycle_length]
                break
            else:
                board_to_index[board] = i
